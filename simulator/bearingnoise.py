...
关键性质（对建模有直接影响）：
  ...
  * 误差只由 (噪声种子, 频道, 测量位置) 决定 → **同一位置同一频道重复测量
    误差完全相同，取平均无法减小误差**
  * 相关长度 ≈ 150 m → 同一网格内的多点误差是同向系统偏差，不能相互抵消
  * salt = 请求频道 → 不同频道是相互独立的误差场
...
def error_degrees(seed: int, salt: int, x_m: float, y_m: float,
    ...
    """官方 bearingnoise.ErrorDegrees：返回示向度误差（度），值域 (-1, 1)。

    seed 为场景噪声种子，salt 为请求频道，x_m/y_m 为机器狗本次测量位置（米）。
    """
    gx = x_m / grid_m
    gy = y_m / grid_m
    ix = math.floor(gx)
...
def quantize_bearing_hundredths(bearing_deg: float, error_deg: float) -> int:
    ...
    先把 (真实示向度 + 误差) 归一化到 [0, 360)，再截断到 0.01°，最后回绕到
    [0, 36000)。加 1e-9 仅为抵消二进制浮点表示误差（如 359.20 存成 359.19999…）。
    """
