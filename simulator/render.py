...
官方不用 encoding/json，而是**手工拼装 JSON 字符串**，因此数字的文本形式
有严格约定，必须逐字节复刻：

  virtual_time_s   → virtualSeconds(µs)：整数时 "%d"（如 105），
                     否则 "%d.%06d" 再去掉尾随零（如 105.500001）
  svd_deg          → "%d.%02d"：恒为两位小数（如 270.02、270.00）
  max_virtual_duration_s / max_real_duration_s → 整数（360000 / 1200）
  remaining_real_duration_s → 整数

这正是 Python 的 json.dumps 做不到的（它会把 105 写成 105.0、把 270.00
写成 270.0），所以这里照官方做法手工拼接；字符串值仍用 json.dumps 转义。
...
def virtual_seconds(virtual_time_us: int) -> str:
    """官方 robotapi.virtualSeconds：微秒 → 无尾随零的秒数字字面量。
...
def render_accepted(path: str, real_timestamp_ms: int, virtual_time_us: int,
    ...
    """渲染 accepted=true 的响应体（官方 renderResult）。"""
